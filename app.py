...
st.dataframe(
    ...
    column_config={
        ...
        "Negative_Outlier": st.column_config.CheckboxColumn("Negative Outlier", width="small"),
        "Positive_Outlier": st.column_config.CheckboxColumn("Positive Outlier", width="small"),
        "Outlier": st.column_config.CheckboxColumn("Outlier", width="small"),
...
if len(positive_outliers) > 0:
    st.markdown("#### Positive Outliers — Reward Eligible")
    st.success(
...
if len(negative_outliers) > 0:
    st.markdown("#### Negative Outliers — Not Reward Eligible")
    st.warning(
